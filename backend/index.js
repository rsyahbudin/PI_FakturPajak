...
const startServer = async () => {
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      const checkTransactionQuery =
        "SELECT TRXNO FROM transaksi WHERE TRXNO = ?";
        ...
        (err, transactionResult) => {
          ...
          db.query(
            ...
            (err, durationResult) => {
              if (err) {
                console.error(
                  "Error checking transaction duration in MySQL:",
                  err
                );
                res.status(500).json({ error: "Internal server error" });
                return;
  ...
  async function fetchDataFromMysqlAndSendEmail(
    TRXNO,
    EMAIL,
    NPWP,
    NAMAPT,
    ...
    try {
      ...
      connection.query(
        checkTransactionQuery,
        [TRXNO],
        (err, checkTransactionResult) => {
          if (err) throw err;

          // If no result, stop and do not send email
          if (checkTransactionResult.length === 0) {
            console.log(
              "Transaction ID not found in MySQL database. Skipping confirmation email."
            );
            return;
          }
